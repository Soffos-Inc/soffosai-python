...
class QuestionAnsweringService(SoffosAIService):
    ...
    def __init__(self, user, src=None, document_ids=None, concern=None, document_text=None,**kwargs) -> None:
        super().__init__(
            service=ServiceString.QUESTION_ANSWERING, user=user, src=src
        )
    ...
    def get_json(self):
        ...
        if self._document_ids:
            request_data = {
                "user": self._user,
                "message": self._src.get("message"),
                "document_ids": self._src.get("document_ids")
            }
        else:
            request_data = {
                "user": self._user,
                "message": self._src.get("message"),
                "document_text": self._src.get("document_text")
            }
