...
output = DocumentsSearchService(user="client_user_id", document_ids=['the_document_id_of_ingested_context'])
print(json.dumps(output, indent=4))

# To delete an ingested content to free up your Soffos storage
output = DocumentsDeleteService(user="client_user_id", document_ids=['the_document_id_of_ingested_context'])
print(json.dumps(output, indent=4))
